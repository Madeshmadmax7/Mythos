...
def _raise_llm_http_error(exc: Exception):
    ...
    if "expired_api_key" in lowered or "invalid api key" in lowered or "401" in lowered:
        raise HTTPException(
            status_code=503,
            detail="LLM API key is invalid or expired. Update LLM_API_KEY and restart backend.",
        )

    if "rate limit" in lowered or "429" in lowered:
        raise HTTPException(
            status_code=429,
            detail="LLM provider rate limit hit. Please retry in a moment.",
        )
